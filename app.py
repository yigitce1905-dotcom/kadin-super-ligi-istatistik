...
<div style="border:1px solid #6366f1;border-radius:12px;padding:16px 20px;margin-bottom:16px;background:#0f172a;">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:14px 18px;font-size:0.88rem;">{_satirlar}</div>
    ...
    # ── 1) KARİYER ────────────────────────────────────────────────────────────
    with st.expander(_bolum_baslik("KARİYER", "CAREER", "kariyer"), expanded=False):
    ...
    # ── 3) ANALİZ ─────────────────────────────────────────────────────────────
    with st.expander(_bolum_baslik("ANALİZ", "ANALYSIS", "analiz"), expanded=False):
...
# ── Profil bölümlerinin erişim kademeleri (kullanıcı gruplarına göre ekran) ──
# Değiştirmek için tek yer burası: "basic" = giriş yapan herkes,
# "pro" ve "premium" = ilgili paketler, admin her şeyi görür.
_BOLUM_TIER = {
    "istatistik": "basic",
    "kariyer":    "basic",
    "gozlem":     "pro",
...
def _bolum_acik(k: str) -> bool:
    return tier_yeterli(_BOLUM_TIER.get(k, "basic"))

def _bolum_baslik(tr_, en_, k: str) -> str:
    return ("" if _bolum_acik(k) else "🔒 ") + t(tr_, en_)

def _bolum_kilit(k: str):
    """Kilitli bölümün gövdesi: hangi üyelikle açılacağını söyler."""
    ger = _BOLUM_TIER.get(k, "basic")
    _ad, _renk, _ikon = _TIER_GORUNUM.get(ger, ("Üyelik", "#8899aa", "🔒"))
    st.markdown(
        f"<div style='text-align:center;padding:22px 16px;color:#8899aa;'>"
        f"<div style='font-size:1.6rem;'>🔒</div>"
        f"<div style='margin-top:6px;font-size:0.9rem;'>"
        + t(f"Bu bölüm {_ikon} <b style='color:{_renk};'>{_ad}</b> üyelikle görüntülenir.",
            f"This section is available with {_ikon} <b style='color:{_renk};'>{_ad}</b> membership.")
        + "</div><div style='font-size:0.78rem;margin-top:6px;color:#64748b;'>"
        + t("Yükseltme için: Profilim → Üyelik", "To upgrade: My Profile → Membership")
        + "</div></div>", unsafe_allow_html=True)
...
def render_ana_lig_profil(secili):
    ...
    if secili and secili in oyuncu_detay:
        ...
        # ── 1) İSTATİSTİKLER ──────────────────────────────────────────────────
        with st.expander(_bolum_baslik("İSTATİSTİKLER", "STATISTICS", "istatistik"), expanded=False):
            if not _bolum_acik("istatistik"):
                _bolum_kilit("istatistik")
            else:
...
# ─── KULÜP KOKPİTİ (2026-27) — ADMIN-ÖZEL iç kadro/kontrat aracı ─────────────
# Veri: kokpit_kadrolar.json (kokpit_sd_cek.py üretir; SD kulüp sayfalarından).
# Erkek tarafındaki ISM kokpitinin kadın uyarlaması — eksik veri açıkça işaretlenir.
_KOKPIT_ULKE = {  # SD bayrak alt'ları Almanca gelir → TR görünüm
    "Türkei": "Türkiye", "Deutschland": "Almanya", "Dänemark": "Danimarka",
    "Portugal": "Portekiz", "Moldawien": "Moldova", "Frankreich": "Fransa",
    "Spanien": "İspanya", "Niederlande": "Hollanda", "England": "İngiltere",
    "Brasilien": "Brezilya", "Österreich": "Avusturya", "Schweiz": "İsviçre",
    "Polen": "Polonya", "Rumänien": "Romanya", "Griechenland": "Yunanistan",
    "Kroatien": "Hırvatistan", "Serbien": "Sırbistan", "Ukraine": "Ukrayna",
    "Georgien": "Gürcistan", "Aserbaidschan": "Azerbaycan", "Marokko": "Fas",
    "Algerien": "Cezayir", "Tunesien": "Tunus", "Schweden": "İsveç",
    "Norwegen": "Norveç", "Belgien": "Belçika", "Italien": "İtalya",
    "Ungarn": "Macaristan", "Tschechien": "Çekya", "Slowakei": "Slovakya",
    "Nordmazedonien": "K. Makedonya", "Japan": "Japonya", "Kolumbien": "Kolombiya",
    "Elfenbeinküste": "Fildişi Sahili", "Südafrika": "G. Afrika", "Sambia": "Zambiya",
    "USA": "ABD", "Kanada": "Kanada", "Nigeria": "Nijerya", "Jamaika": "Jamaika",
}

def _kokpit_ulke(s: str) -> str:
    return " / ".join(_KOKPIT_ULKE.get(p.strip(), p.strip()) for p in (s or "").split("/") if p.strip())

@st.cache_data(show_spinner=False, ttl=300)
def kokpit_yukle() -> dict:
    _yol = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kokpit_kadrolar.json")
    try:
...
_TRANSFER_DB = {
    ...
    ("Kaleci", "Yuksek", "Yabancı"):    ["NATALIA MUNTEANU", "MARIA ASUNCION QUINONES GOICOECHEA", "ROBERTA APRILE"],
