...
# Sultanbeyli Fatih Vatan shortlist: SADECE Defans + Hücum
GRUPLAR = [("DEF","DEFANS","DEFENDERS"), ("FW","HÜCUM","FORWARDS")]
_gruplar_set = {g for g, _, _ in GRUPLAR}
OYUNCULAR = [o for o in OYUNCULAR if o[2] in _gruplar_set]
...
pdf.cell(0, 11, "SHORTLIST", ln=1)
pdf.set_x(14); pdf.set_font("DV", "B", 10.5); pdf.set_text_color(*OLIV)
pdf.cell(0, 6, "SULTANBEYLİ FATİH VATAN SPOR · DEFANS & HÜCUM · YAZ 2026", ln=1)
pdf.set_x(14); pdf.set_font("DV", "", 9); pdf.set_text_color(*GRIM)
pdf.cell(0, 6, "Kadro planlamanız için seçilmiş, transfere açık defans ve hücum oyuncularımız", ln=1)
...
_ozet = [(str(len(OYUNCULAR)), "OYUNCU"), (str(len(_ulkeler)), "FARKLI ÜLKE"),
         (f"{_say['DEF']}", "DEFANS"), (f"{_say['FW']}", "HÜCUM")]
...
def _grup_sayfa_ac(tr, en, adet):
    pdf.add_page(); zemin()
    marka_bandi(h=13, baslik="SHORTLIST · SULTANBEYLİ FATİH VATAN · YAZ 2026")
...
cikti = pathlib.Path.home() / "Desktop" / "ISM_Fatih_Vatan_Shortlist_2026.pdf"
