# -*- coding: utf-8 -*-
"""
U13 Minik Kızlar (İstanbul) scraper — tffistanbul.org
Gruplar 1/2/3 → her hafta → her maç-detayı → İlk 11 + Yedekler oyuncuları topla.
Çıktı altlig_u13.json, altlig_u17.json ile BİREBİR aynı şemada (app reuse eder).
(scraper_u15.py'nin U13 sürümü — yapı aynı; config farklı: lig yolu, gruplar, maç süresi.)
...
Kullanım:  python scraper_u13.py
"""
...
LIG_BASE = BASE + "/puantaj-ve-fikstur/2025-2026/u13-minik-kizlar/29/306"
GRUPLAR  = {"1": 6044, "2": 6045, "3": 6046}
MAC_SURE = 60   # U13 Minik Kızlar: 2×30 = 60 dk (olaylar 60'ta kümeleniyor, 63=60+uzatma)
...
def main():
    ...
    for kid, r in oy.items():
        ...
        oyuncular.append({
            ...
            "tum_takimlar": " / ".join(tks), "grup": r["grup"], "lig": "U13 Minik Kızlar",
    ...
    out = {
        "lig": "U13 Minik Kızlar Ligi", "kategori": "U13",
        "guncelleme": datetime.date.today().isoformat(),
        "gol_kralicesi": golcu, "oyuncular": oyuncular,
    }
    json.dump(out, open("altlig_u13.json", "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    print(f"\n[OK] {len(oyuncular)} oyuncu, {toplam_mac} maç -> altlig_u13.json")
