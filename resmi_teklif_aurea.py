# -*- coding: utf-8 -*-
"""Haymana Spor Kulübü — Aurea del Carmen RESMÎ TRANSFER TEKLİFİ (iki dilli, Bellamy/Ellis/Markovska şablonu)."""
import sys, pathlib
...
# ── OYUNCU KÜNYESİ ──
OYUNCU   = "Aurea del Carmen"
UYRUK    = "ABD / United States"
DOGUM    = "13.07.2001"            # 13 Temmuz 2001
TARIH    = "05.08.2026"

# ── MALİ ŞARTLAR ──
AY       = 9
AYLIK    = 1000                   # USD net / ay
TOPLAM   = AY * AYLIK             # 9.000 USD
KOMISYON = round(TOPLAM * 0.10)   # %10 = 900 USD
ODEME_BASLANGIC_TR = "Eylül 2026"
ODEME_BASLANGIC_EN = "September 2026"
ODEME_BITIS_TR     = "Mayıs 2027"
ODEME_BITIS_EN     = "May 2027"
KOMISYON_SON_TR    = "1 Eylül 2026"
KOMISYON_SON_EN    = "September 1, 2026"

# ── MENAJERLİK / ARACILIK (Sydney Bellamy sözleşmesiyle aynı yapı) ──
...
def usd(n):    return f"{n:,}".replace(",", ".")   # TR: 9.000
def usd_en(n): return f"{n:,}"                     # EN: 9,000
...
para(f"Maaş ödemeleri, {ODEME_BASLANGIC_TR} - {ODEME_BITIS_TR} döneminde olmak üzere, dokuz eşit "
     "aylık ödeme hâlinde gerçekleştirilecektir. Ödemeler USD olarak yapılacaktır. Farklı bir ödeme "
     "şekli ancak tarafların yazılı mutabakatıyla uygulanabilir.")
...
para(f"The salary will be paid in nine equal monthly instalments, covering the period from "
     f"{ODEME_BASLANGIC_EN} to {ODEME_BITIS_EN}. Payments will be made in USD unless otherwise "
...
# ── 7. MENAJERLİK KOMİSYONU / ARACILIK ──
bolum("7. MENAJERLİK KOMİSYONU / AGENT COMMISSION")
para(f"Bu transferin gerçekleşmesi ve futbolcu sözleşmesinin imzalanması hâlinde Haymana Spor "
     f"Kulübü, oyuncunun toplam sözleşme bedelinin yüzde 10'una karşılık gelen {usd(KOMISYON)} USD "
     f"menajerlik komisyonunu, oyuncunun temsilcisi {AJAN_SIRKET} (yetkili: {AJAN_YETKILI}, "
     f"{AJAN_FIFA})'a ödeyecektir.")
para(f"Menajerlik komisyonu en geç {KOMISYON_SON_TR} tarihine kadar ödenecektir.")
para(f"Bu transferde aracı (Intermediary) olarak {INTERMEDIARY} görev yapmaktadır.")
para(f"Menajerlik komisyonu, oyuncuya ödenecek toplam {usd(TOPLAM)} USD tutarındaki maaştan ayrı "
     f"olarak kulüp tarafından ödenecek ve oyuncunun maaşından herhangi bir kesinti yapılmayacaktır.")
para(f"Upon completion of the transfer and execution of the football player contract, Haymana "
     f"Sports Club will pay an agent commission of USD {usd_en(KOMISYON)}, corresponding to 10% of "
     f"the player's total contractual salary, to {AJAN_SIRKET} (represented by {AJAN_YETKILI}, "
     f"{AJAN_FIFA}), the player's representative.")
para(f"The agent commission shall be paid no later than {KOMISYON_SON_EN}.")
para(f"{INTERMEDIARY} acts as the intermediary in this transfer.")
para(f"The agent commission will be paid separately by the club and will not be deducted from the "
     f"player's total salary of USD {usd_en(TOPLAM)}.")

# ── 8. GENEL HÜKÜMLER ──
bolum("8. GENEL HÜKÜMLER / GENERAL CONDITIONS")
...
_kase_y = pdf.get_y()
para("Kaşe / Stamp:", bold=True, gap=3)
KASE_GORSEL = pathlib.Path(__file__).parent / "haymana_kase.png"
if KASE_GORSEL.exists():
    pdf.image(str(KASE_GORSEL), x=95, y=_kase_y - 2, w=32)
...
cikti = pathlib.Path.home() / "Desktop" / "Resmi_Teklif_Haymana_Aurea.pdf"
