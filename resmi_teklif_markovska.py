# -*- coding: utf-8 -*-
"""Haymana Spor Kulübü — Aleksandra Markovska RESMÎ TRANSFER TEKLİFİ (iki dilli, Kaio şablonu)."""
import sys, pathlib
from fpdf import FPDF

sys.stdout.reconfigure(encoding="utf-8")

# ── OYUNCU KÜNYESİ (Markovska scout DB'de yok — elle girildi) ──
OYUNCU   = "Aleksandra Markovska"
UYRUK    = "Kuzey Makedonya / North Macedonia"
DOGUM    = "04.05.1997"
TARIH    = "22.07.2026"

# ── MALİ ŞARTLAR ──
AY       = 9
AYLIK    = 800                    # USD net / ay
TOPLAM   = AY * AYLIK             # 7.200 USD
KOMISYON = round(TOPLAM * 0.10)   # %10 = 720 USD

# ── MENAJERLİK / ARACILIK (Paola/Ellis ile aynı yapı) ──
AJAN        = "Zoran Veljanovski"
AJAN_FIFA   = "FIFA ID 202405-6005"
AJAN_SIRKET = "ASSA Sport Talent"
INTERMEDIARY = "Ideal Sports Management"

def usd(n):    return f"{n:,}".replace(",", ".")   # TR: 7.200
def usd_en(n): return f"{n:,}"                     # EN: 7,200
...
para(f"Bu transferin gerçekleşmesi ve futbolcu sözleşmesinin imzalanması hâlinde Haymana Spor "
     ...
     f"menajerlik komisyonunu, oyuncunun temsilcisi {AJAN} ({AJAN_FIFA}) — {AJAN_SIRKET}'a "
     f"ödeyecektir.")
...
para(f"Upon completion of the transfer and execution of the football player contract, Haymana "
     ...
     f"the player's total contractual salary, to {AJAN} ({AJAN_FIFA}) — {AJAN_SIRKET}, the player's "
     f"representative.")
...
cikti = pathlib.Path.home() / "Desktop" / "Resmi_Teklif_Haymana_Markovska.pdf"
